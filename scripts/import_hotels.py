...
CATEGORY_ID = 150093  # Hotels & Travel (parent)
SUBCATEGORY_ID = 150148  # Lodges/Hotels
ADMIN_USER_ID = 1
DEFAULT_IMAGE = 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=640'
...
def import_hotels():
    with open('hingoli_hotels.json', 'r', encoding='utf-8') as f:
        hotels = json.load(f)
    ...
    for hotel in hotels:
        title = hotel.get('name', '').strip()
        # Remove special characters like ₹ that cause encoding issues
        ...
        address = clean_address(hotel.get('address', ''))
        if not address:
            address = 'Hingoli'
        
        phone = extract_phone(hotel.get('phone', ''))
        rating = float(hotel.get('rating') or 0)
        review_count = int(hotel.get('review_count') or 0)
        images = [img for img in hotel.get('images', []) if 'jdmagicbox.com/v2/comp' in img or 'jdmagicbox.com/comp' in img]
        ...
        if images:
            filename = f"listings/hotels/{name_slug}_{timestamp}.webp"
