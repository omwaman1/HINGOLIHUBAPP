"""
Import Basmath Hospitals (and any Hingoli ones in this file)
"""
...
SUBCATEGORY_ID = 150121  # Hospital
ADMIN_USER_ID = 1
DEFAULT_IMAGE = 'https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?w=640'
...
def import_hospitals():
    with open('basmath_hospitals.json', 'r', encoding='utf-8') as f:
        hospitals = json.load(f)
    
    # Filter only Basmath or Hingoli hospitals
    valid_hospitals = []
    for h in hospitals:
        addr = h.get('address', '').lower()
        if 'basmath' in addr or 'hingoli' in addr or not addr:
            valid_hospitals.append(h)
    
    print(f"Found {len(valid_hospitals)} Basmath/Hingoli hospitals out of {len(hospitals)} total")
    ...
    for hospital in valid_hospitals:
        ...
        address = clean_address(hospital.get('address', ''))
        if not address:
            address = 'Basmath'
        
        # Determine city
        orig_addr = hospital.get('address', '').lower()
        city = 'Basmath' if 'basmath' in orig_addr else 'Hingoli'
        
        phone = extract_phone(hospital.get('phone', ''))
        rating = float(hospital.get('rating') or 0)
        review_count = int(hospital.get('review_count') or 0)
        images = [img for img in hospital.get('images', []) if 'jdmagicbox.com/v2/comp' in img or 'jdmagicbox.com/comp' in img]
        ...
        if images:
            filename = f"listings/hospitals/{name_slug}_{timestamp}.webp"
        ...
        """, (listing_id, 'business', title, address, CATEGORY_ID, SUBCATEGORY_ID,
            address, city, 'Maharashtra', 'India', main_image_url, ADMIN_USER_ID, 
        ...
        for idx, img_url in enumerate(images[1:3], 1):
            gallery_filename = f"listings/hospitals/{name_slug}_{timestamp}_{idx}.webp"
