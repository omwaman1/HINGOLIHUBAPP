...
// Remove /apiv6, /apiv5, /apiv4 or /api prefix if present (adjust based on your hosting setup)
$path = preg_replace('#^/apiv6#', '', $path);
...
function routeRequest(string $method, array $segments): void {
    ...
    switch ($resource) {
        ...
        case 'reel':
            // Serve HTML landing page for shared reel links (not API)
            serveReelLandingPage(array_slice($segments, 1));
            break;
...
// transformShopProduct() is defined in helpers/transformers.php - removed duplicate
...
/**
 * Serve HTML landing page for shared reel links
 * Route: /reel/{id}
 */
function serveReelLandingPage(array $segments): void {
    $reelId = $segments[0] ?? null;
    $reel = null;
    $error = null;
...
<head>
    ...
    if ($reel && !empty($reel['thumbnail_url'])) {
        echo '<meta property="og:image" content="' . htmlspecialchars($reel['thumbnail_url']) . '">';
    }
...
<div class="container">
    ...
    if ($error) {
        ...
        echo '<div class="reel-preview">
            <div class="reel-title">' . htmlspecialchars($reel['title'] ?? 'Untitled Reel') . '</div>
            <div class="reel-stats"><span>❤️ ' . number_format($reel['likes_count'] ?? 0) . ' likes</span></div>
        </div>';
    }
    
    echo '<a href="' . $deepLink . '" class="btn btn-primary" id="openApp">📱 Open in Hingoli Hub</a>
    <br>
    <a href="' . $playStoreUrl . '" class="btn btn-secondary">⬇️ Download App</a>
